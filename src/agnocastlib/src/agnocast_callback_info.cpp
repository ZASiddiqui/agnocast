  ...
  for (int32_t i = static_cast<int32_t>(receive_args.ret_entry_num) - 1; i >= 0; i--) {
...

    {
      ...
      ready_agnocast_executables.emplace_back(
        AgnocastExecutable{callable, callback_info.callback_group, receive_args.ret_entry_ids[i],callback_info});
    }
    else{
      agnocast::decrement_rc(
        ready_agnocast_executables[0].callback_info.topic_name, ready_agnocast_executables[0].callback_info.subscriber_id,
        ready_agnocast_executables[0].entry_id);
